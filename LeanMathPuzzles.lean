import VersoManual
import Verso.Output.Html
import LeanMathPuzzles.Basic
import LeanMathPuzzles.Problem_001

open Verso.Genre Manual

open Verso.Genre.Manual.InlineLean

open LeanMathPuzzles

#doc (Manual) "Lean Mathematical Puzzles" =>

Each chapter explores a distinct puzzle, presented in chronological order according to its time of writing.
The chapters stand independently and need not be read sequentially.

{include 1 LeanMathPuzzles.Problem_001}
